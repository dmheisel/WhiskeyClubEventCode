...
function getSecondToLastThursday(month, year) {
    ...
    var counter = 0;
    for (var i = daysInMonth; i > 0; i--) { // count down from the last day of the month.
        var date = new Date(year, month - 1, i) // create a new Date object for each day of the month.  
        if (date.getDay() === 4) { // date.getDay() returns 0 - 6 for the day of the week, 0 = Sunday.  Thursday will be a 4.
            counter += 1
        };
        if (counter === 2) { // the second time we increment the counter, that's our second-to-last thursday!
            return date
        }
    }
}
